...
По заданию был сделан вывод, что так называемый build.in это просто некий тип БД, таблицы как в экселе,
 ...
 Настоящее время....  
 Было выяснено, что build.in это buildin.ai.  
 Я искренне и честно пыталась впитать в себя максимум их невероятной документации, однако...  
 Я либо что-то НЕРЕАЛЬНО не понимаю, либо такого функционала нет в целом. В BUILDIN AI НЕТ ФОРМУЛ.  
 ВООБЩЕ.   
 И ФУНКЦИЙ НИКАКИХ НЕТ.   
 Однако это наименьшая из наших проблем. Отдельаня беда это заставить прогу работать с выданным мне токеном.  
 Возможно из-за того, что у меня нет прав админа, мне возвращается 404...
...
Приложение использует .env поэтому делаем:  
```
BUILDIN_AI_TOKEN=your_token_here
```
